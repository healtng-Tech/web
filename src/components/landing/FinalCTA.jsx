...
export const FinalCTA = memo(function FinalCTA() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const startActivation = windowHeight;
      const endActivation = windowHeight * 0.25; 
      const totalDistance = startActivation - endActivation;
      const currentDistance = startActivation - rect.top;

      const progress = Math.max(0, Math.min(1, currentDistance / totalDistance));
      const currentWindowWidth = window.innerWidth;
      const baseMaxWidth = 1152; 
      
      const targetMaxWidth = currentWindowWidth > baseMaxWidth 
        ? baseMaxWidth + (currentWindowWidth - baseMaxWidth) * progress 
        : currentWindowWidth;

      const currentRadius = 32 * (1 - progress);
      const currentPaddingH = 24 * (1 - progress);

      cardRef.current.style.maxWidth = `${targetMaxWidth}px`;
      cardRef.current.style.borderTopLeftRadius = `${currentRadius}px`;
      cardRef.current.style.borderTopRightRadius = `${currentRadius}px`;
      cardRef.current.style.borderBottomLeftRadius = '0px'; 
      cardRef.current.style.borderBottomRightRadius = '0px';
  ...
  return (
    <section ref={sectionRef} className="bg-white w-full pt-10 md:pt-16 flex flex-col items-center overflow-hidden" aria-labelledby="cta-heading">
      <div
        ref={cardRef}
        className="bg-gradient-to-b from-[#B2D0FD] to-[#F3F6FF] relative overflow-hidden transition-all duration-75 ease-out"
        style={{ maxWidth: '1152px', borderTopLeftRadius: '32px', borderTopRightRadius: '32px', borderBottomLeftRadius: '0px', borderBottomRightRadius: '0px', width: 'calc(100% - 48px)' }}
      >
        <div className="absolute inset-0 pointer-events-none overflow-hidden" aria-hidden="true">
          <div className="absolute -top-32 -left-32 size-[500px] rounded-full bg-white/40 blur-[80px]" />
          <div className="absolute -bottom-20 -right-20 size-[400px] rounded-full bg-blue-300/10 blur-[100px]" />
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-6 py-20 md:py-28 flex flex-col items-center text-center gap-8">
            ...
            {status === 'success' ? (
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <input required type="text" placeholder="Institución o profesional" value={formData.institution} onChange={(e) => setFormData({...formData, institution: e.target.value})} className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 placeholder:text-slate-400 text-[14px] font-medium outline-none focus:border-[#002EE5] focus:bg-white transition-all duration-150" />
                  <input required type="text" placeholder="Especialidad o sector" value={formData.specialty} onChange={(e) => setFormData({...formData, specialty: e.target.value})} className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 placeholder:text-slate-400 text-[14px] font-medium outline-none focus:border-[#002EE5] focus:bg-white transition-all duration-150" />
                </div>
                <input required type="tel" placeholder="WhatsApp (+58)" value={formData.whatsapp} onChange={(e) => setFormData({...formData, whatsapp: e.target.value})} className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 placeholder:text-slate-400 text-[14px] font-medium outline-none focus:border-[#002EE5] focus:bg-white transition-all duration-150" />
          ...
          <div className="flex flex-wrap items-center justify-center gap-x-8 gap-y-3 mt-2">
            {[
              { icon: '🔒', text: 'Datos protegidos bajo ley venezolana' },
              { icon: '⚡', text: 'Activación inmediata el mismo día' },
              { icon: '🏥', text: 'Despliegue regional: Carabobo' },
            ].map((item, i) => (
              <div key={i} className="flex items-center gap-2 text-[12px] text-slate-800 font-semibold">
